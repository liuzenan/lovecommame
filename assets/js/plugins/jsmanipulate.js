...
function LomoStyle(){
	...
	this.exposure = new ExposureFilter();
	this.color = new SaturationFilter();
	this.filter = function(input,values){
		
		this.exposure.filter(input,values);
		this.color.filter(input,values);
		this.color.filter(input,values);
		this.vignette.filter(input,values);
	};
}


function pencilSketchFilter(){
	this.name = "Pencil Sketch";
	this.isDirAnimatable = false;
	this.defaultValues = {
		amount : 25,
		exposure : 3.0,
	...
	this.smear = new LineSmearFilter();
	this.noise = new NoiseFilter();
	this.exposure = new ExposureFilter();
	this.grayscale = new GrayscaleFilter();
	this.filter = function(input,values){
		this.grayscale.filter(input,values);
		this.exposure.filter(input,values);
		this.noise.filter(input,values);
		this.smear.filter(input,values);
	};
}
...
$.JSManipulate = {
	...
	lomo: new LomoStyle(),
	pencilsketch: new pencilSketchFilter()
};
